/**
 * Relay inbound Cloud API (webhook Meta do App Drax) → Soma ChatBot.
 *
 * Copiar para o repositório WABA:
 *   src/integrations/soma/soma-chatbot-cloud-relay.ts
 *
 * Registrar em src/index.ts (junto das rotas /integrations/soma/*):
 *   registerSomaChatbotCloudRelayRoutes(app);
 *
 * No MetaWhatsappWebhookService.processPostedEvent, depois do loop de events:
 *   void relaySomaChatbotCloudWebhook(payload);
 *
 * Env WABA:
 *   SOMA_WABA_INTEGRATION_KEY   (já existe)
 *   SOMA_CHAT_CLOUD_WEBHOOK_URL (opcional; senão usa a URL enviada no register)
 */
